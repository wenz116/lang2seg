...
class GtMRCNLoader(Loader):
  ...
  def getBatch(self, split, opt):
    ...
    # fetch feats
    batch_ref_ids = []
    batch_pos_ann_ids, batch_pos_sent_ids, batch_pos_pool5, batch_pos_fc7, batch_pos_mask_feat = [], [], [], [], []
    batch_pos_cxt_fc7, batch_pos_cxt_lfeats = [], []
    batch_neg_ann_ids, batch_neg_sent_ids, batch_neg_pool5, batch_neg_fc7, batch_neg_mask_feat = [], [], [], [], []
    ...
    for image_id in batch_image_ids:
      ...
      image_pos_pool5, image_pos_fc7, image_pos_mask_feat = self.fetch_grid_feats(pos_ann_boxes, head, im_info)  # (num_pos, k, 7, 7)
      batch_pos_pool5 += [image_pos_pool5]
      batch_pos_fc7   += [image_pos_fc7]
      batch_pos_mask_feat  += [image_pos_mask_feat]
      neg_ann_boxes = xywh_to_xyxy(np.vstack([self.Anns[ann_id]['box'] for ann_id in image_neg_ann_ids]))
      image_neg_pool5, image_neg_fc7, image_neg_mask_feat = self.fetch_grid_feats(neg_ann_boxes, head, im_info)  # (num_neg, k, 7, 7)
      batch_neg_pool5 += [image_neg_pool5]
      batch_neg_fc7   += [image_neg_fc7]
      batch_neg_mask_feat  += [image_neg_mask_feat]
    ...
    pos_mask_feat  = torch.cat(batch_pos_mask_feat, 0); pos_mask_feat.detach()
    ...
    neg_mask_feat  = torch.cat(batch_neg_mask_feat, 0); neg_mask_feat.detach()
    ...
    data['Feats'] = {'mask_feat': pos_mask_feat, 'fc7': pos_fc7, 'pool5': pos_pool5, 'lfeats': pos_lfeats, 'dif_lfeats': pos_dif_lfeats,
    ...
    data['neg_Feats'] = {'mask_feat': neg_mask_feat, 'fc7': neg_fc7, 'pool5': neg_pool5, 'lfeats': neg_lfeats, 'dif_lfeats': neg_dif_lfeats,
  ...
  def fetch_grid_feats(self, boxes, net_conv, im_info):
    ...
    - fc7   (n, 2048, 7, 7) 
    """
    pool5, fc7, mask_feat = self.mrcn.box_to_spatial_fc7(net_conv, im_info, boxes)
    return pool5, fc7, mask_feat
  ...
  def getAttributeBatch(self, split):
    ...
    pool5, fc7, mask_feat = self.fetch_grid_feats(ann_boxes, head, im_info)  # (#ann_ids, k, 7, 7)
    ...
    data['Feats'] = {'mask_feat': mask_feat, 'pool5': pool5, 'fc7': fc7, 'lfeats': lfeats, 'dif_lfeats': dif_lfeats}
    data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
    return data
