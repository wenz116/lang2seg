...
from model.config_vgg import cfg
    ...
    while iter < max_iters + 1:
      ...
      for idx in range(sent_num):
        ...
        if iter == 1 or iter % 500 == 0: #now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL: ####
          ...
          rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, total_loss, summary = \
          ...
          #for key, value in dict(self.net.named_parameters()).items():
          #  if 'layer3.18' in key and 'weight' in key and value.requires_grad:
          #    print(key, value)

        else:
          # Compute the graph without summary
          rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, total_loss = \
        ...
        if iter % (cfg.TRAIN.DISPLAY) == 0:
          print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> loss_response: %.6f\n >>> lr: %f' % \
                (iter, max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, lr))
