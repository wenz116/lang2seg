...
class SolverWrapper(object):
  ...
  def from_snapshot(self, sfile, nfile):
    ...
    for name, param in new_params.items():
      ...
      if param.size() == saved_state_dict[name].size():
  ...
  def train_model(self, max_iters):
    ...
    while iter < max_iters + 1:
      ...
      dynamic_filter = Variable(torch.ones(1, 1024, 1, 1).cuda())

      now = time.time()
      if 0:#now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL:
        # Compute the graph with summary
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, total_loss, summary = \
          self.net.train_step_with_summary(blobs, self.optimizer, dynamic_filter)
        for _sum in summary: self.writer.add_summary(_sum, float(iter))
        # Also check the summary on the validation set
        blobs_val = self.data_layer_val.forward()
        summary_val = self.net.get_summary(blobs_val, dynamic_filter)
        ...
        rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, total_loss = \
          self.net.train_step(blobs, self.optimizer, dynamic_filter)
