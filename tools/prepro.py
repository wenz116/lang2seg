...
def prepare_json(refer, sentToFinal, ref_to_att_wds, params):
  coco_cat_id_to_class_ind = {coco_cat_id: class_ind for class_ind, coco_cat_id in enumerate([0] + refer.Cats.keys())} ####
  ...
  for ref_id, ref in refer.Refs.items():
    ...
    rle = refer.getMask(ref)['rle']
    #print(rle, len(rle), '--------')
    #mask = refer.getMask(ref)['mask'].tolist() ####
    att_wds = ref_to_att_wds[ref_id] if ref_id in ref_to_att_wds else []
    refs += [{'ref_id': ref_id, 'split': ref['split'], 'category_id': coco_cat_id_to_class_ind[ref['category_id']], 'ann_id': ref['ann_id'], ####
              'sent_ids': ref['sent_ids'], 'box': box, 'rle': rle, 'image_id': ref['image_id'],
              'att_wds': att_wds} ]
  print('There are in all %s refs.' % len(refs))
  ...
  for image_id in refer.Imgs:
    ...
    for ann_id in ann_ids:
      ...
      anns += [{'ann_id': ann_id, 'category_id': coco_cat_id_to_class_ind[ann['category_id']], 'box': ann['bbox'], 'image_id': image_id, 'h5_id': h5_id}] ####
      h5_id += 1
...
def main(params):
  ...
  json.dump({'refs': refs, ####
             ...
             'att_to_ix' : atoi,
             'att_to_cnt': att2cnt,
             'cat_to_ix': {cat_name: coco_cat_id_to_class_ind[cat_id] for cat_id, cat_name in refer.Cats.items()}, ####
