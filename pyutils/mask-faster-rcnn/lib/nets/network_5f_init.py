...
class Network(nn.Module):
  ...
  def _predict(self, save=0):
    ...
    mask_1[:, :, :int(mask_1.size(2)/2), :int(mask_1.size(3)/2)] = 1
    ...
    mask_2[:, :, :int(mask_2.size(2)/2), int(mask_2.size(3)/2):] = 1
    ...
    mask_3[:, :, int(mask_3.size(2)/2):, :int(mask_3.size(3)/2)] = 1
    ...
    mask_4[:, :, int(mask_4.size(2)/2):, int(mask_4.size(3)/2):] = 1
    net_conv_4 = net_conv * mask_4
    response_4 = F.conv2d(net_conv_4, dynamic_filter_4) ####
    
    response = torch.cat((response_0, response_1, response_2, response_3, response_4), 1)
