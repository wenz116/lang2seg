...
class CycleLoader(Loader):
  ...
  def _get_image_blob(self, image_ids):
  ...
  def getBatch(self, split, batch_size=1): #, opt):
    ...
    for i in range(batch_size):
      ...
      if ri_next > max_index:
        print('number of images in split {}: {}'.format(split, len(split_ix)))
    ...
    for image_id in batch_image_ids:
      ...
      for ref_id in ref_ids:
        ...
        ref_mask = imresize(ref_mask, size=(blob.shape[1], blob.shape[2]), interp='nearest')
      ...
      pos_ref_boxes = xywh_to_xyxy(np.vstack([self.Refs[ref_id]['box'] for ref_id in batch_pos_ref_ids]))
      #print('----------------------')
      #print(pos_ann_boxes)
      #print(pos_ref_boxes) #### ++++
    ...
    label_batch = np.zeros([cap_labels.shape[0], cap_labels.shape[1] + 2], dtype='int')
    mask_batch = np.zeros([cap_labels.shape[0], cap_labels.shape[1] + 2], dtype='float32')
    ...
    data = {}
    data['data'] = blob
    data['im_info'] = np.array([[blob.shape[1], blob.shape[2], im_scales[0]]]).astype(np.float32)
    
    pos_ref_boxes = np.concatenate((pos_ref_boxes * im_scales[0], np.array([batch_pos_category_ids]).T), axis=1)
  ...
  def getAttributeBatch(self, split):
    ...
    C4_feat    = head.repeat(len(ann_boxes),1,1,1)
    ...
    data['Feats'] = {'C4_feat': C4_feat, 'pool5': pool5, 'fc7': fc7, 'lfeats': lfeats, 'dif_lfeats': dif_lfeats}
    data['bounds'] = {'it_pos_now': self.iterators[split], 'it_max': max_index, 'wrapped': wrapped}
    return data
  ...
  def getTestBatch(self, split): # revised from getBatch
    ...
    data['bounds'] = {'it_pos_now': ri, 'it_max': max_index, 'wrapped': wrapped}
  ...
  def getTestBatch_0(self, split, opt):
