...
class Network(nn.Module):
  ...
  def _predict(self, save=0): #, dynamic_filter):
    """Return
    - net_conv : (1, 1024, H, W)
    - rois     : (n, 5) [0xyxy]
    ...
    # dynamic filter
    hidden = torch.mean(hidden, 0, True)
    dynamic_filter = F.tanh(self.dynamic_fc(hidden))
    dynamic_filter = dynamic_filter.view(1, -1, 1, 1)
    
    response = F.conv2d(net_conv, dynamic_filter) ####
    net_conv = torch.cat((net_conv, response), 1) ####
    #response: (1L, 1L, 38L, 57L) net_conv: (1L, 1025L, 38L, 57L)
    ...
    if save:
      ...
      if not os.path.exists(im_dir):
        os.makedirs(im_dir)
      #kkk = len([name for name in os.listdir(im_dir) if os.path.isfile(os.path.join(im_dir, name))])
      #print(kkk)
      print(self._file_name)
      im.save('{}/{}_r.png'.format(im_dir, self._file_name[:-4]))
    ...
    #rois: (256L, 5L) pool5: (256L, 1025L, 7L, 7L)
    ...
    #net_conv: (1L, 1025L, 38L, 57L) rois: (256L, 5L) cls_prob: (256L, 81L) bbox_pred: (256L, 324L) mask_prob: (36L, 81L, 14L, 14L)
  ...
  def forward(self, image, im_info, gt_boxes=None, gt_masks=None, labels=None, file_name=None, mode='TRAIN', save=0):
    ...
    # image: -122.772 ~ 152.02, shape: (1, 600, 899, 3) PIXEL_MEANS = np.array([[[102.9801, 115.9465, 122.7717]]])
    # im_info: [[ 600. 899. 1.4051522]], shape: (1, 3) shorter border length: 600, longer border length < 1000
    # gt_boxes: (n, 5), [x1, y1, x2, y2, cls] ####
    # [[ 699.76580811  231.85011292  792.50585938  525.5269165   1.]
    # [ 472.13113403  234.66041565  484.77752686  248.71194458  1.]
    # [ 422.95080566  236.06556702  435.59719849  250.11709595  1.]
    # [ 366.74472046  238.87588501  382.20141602  252.92739868  1.]
    # [ 331.61593628  230.44496155  347.07260132  250.11709595  1.]
    # [ 386.41687012  234.66041565  397.65808105  252.92739868  1.]
    # [ 451.05386353  230.44496155  472.13113403  250.11709595  1.]
    # [ 491.80328369  233.25526428  498.82904053  248.71194458  1.]
    # [ 462.29507446  233.25526428  476.34661865  250.11709595  1.]
    # [ 377.98596191  236.06556702  387.82202148  251.52224731  1.]
    # [ 418.73535156  233.25526428  428.57144165  244.49649048  1.]
    # [ 299.29742432  200.93676758  629.50817871  310.53863525  6.]], shape: (12, 5)
    # gt_masks: (n, scaled_height, scaled_width) (12, 600, 899)
  ...
  def test_image(self, image, im_info, labels):
    """Return 
    ...
    self.forward(image, im_info, None, None, labels, file_name, mode='TEST')
  ...
  def get_summary(self, blobs): #, dynamic_filter):
    self.eval()
  ...
  def train_step(self, blobs, train_op): #, dynamic_filter):
  ...
  def train_step_with_summary(self, blobs, train_op): #, dynamic_filter):
    #self.forward(blobs['data'], blobs['im_info'], dynamic_filter, blobs['gt_boxes'], blobs['gt_masks'])
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'], blobs['gt_masks'], blobs['labels'], blobs['file_name'], save=1)
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss = self._losses['rpn_cross_entropy'].data[0], \
                                                                      self._losses['rpn_loss_box'].data[0], \
                                                                      self._losses['cross_entropy'].data[0], \
                                                                      self._losses['loss_box'].data[0], \
