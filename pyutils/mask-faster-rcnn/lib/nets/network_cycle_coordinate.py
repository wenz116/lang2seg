...
class Network(nn.Module):
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    pred_boxes = Variable(torch.zeros(1, 1, feats_all.size(2), feats_all.size(3)).cuda(), requires_grad=True)
    print('0:', pred_boxes.requires_grad)
    #print('pred_boxes:', pred_boxes.size())
    r1 = int(round(self._pred_boxes[1]/(self._im_info[0][0]-1)*(feats_all.size(2)-1)))
    r2 = int(round(self._pred_boxes[3]/(self._im_info[0][0]-1)*(feats_all.size(2)-1)))
    c1 = int(round(self._pred_boxes[0]/(self._im_info[0][1]-1)*(feats_all.size(3)-1)))
    c2 = int(round(self._pred_boxes[2]/(self._im_info[0][1]-1)*(feats_all.size(3)-1)))
    if r1 < r2 and c1 < c2:
      pred_boxes[:, :, r1:r2, c1:c2] = 1
    else:
      print('---------- empty box ----------')
    print('1:', pred_boxes.requires_grad)
