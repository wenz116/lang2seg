...
class ResNet(nn.Module):
  def __init__(self, block, layers, num_classes=1000):
    ...
    self.layer4 = self._make_layer(block, 512, layers[3], stride=1, add_dim=1) ####
  ...
  def _make_layer(self, block, planes, blocks, stride=1, add_dim=0): ####
    downsample = None
    if stride != 1 or self.inplanes != planes * block.expansion:
      downsample = nn.Sequential(
        #nn.Conv2d(self.inplanes, planes * block.expansion,
        nn.Conv2d(self.inplanes+add_dim, planes * block.expansion, ####
    ...
    layers.append(block(self.inplanes+add_dim, planes, stride, downsample)) ####
...
class resnetv1(Network):
  ...
  def _init_modules(self):
    ...
    self.rpn_net = nn.Conv2d(1025, 512, [3, 3], padding=1) ####
