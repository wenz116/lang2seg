...
class SolverWrapper(object):
  ...
  def from_snapshot(self, sfile, nfile):
    ...
    with open(nfile, 'rb') as fid: #### need to restore image id
      st0 = pickle.load(fid)
      st1 = pickle.load(fid)
    ...
    if cfg.TRAIN.FROM_FRCN:
      ...
      for key, value in dict(self.net.named_parameters()).items():
        if value.requires_grad: # layer1 (when RESNET.FIXED_BLOCKS = 1), bn, downsample excluded
          if ('rnn_encoder' in key or 'dynamic_fc' in key) and 'bias' in key:
          ...
          elif 'rnn_encoder' in key or 'dynamic_fc' in key:
  ...
  def train_model(self, max_iters):
    ...
    while iter < max_iters + 1:
      ...
      now = time.time()
      if iter == 1 or iter % 200 == 0: #now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL: ####
        print('----------- summary -----------')
