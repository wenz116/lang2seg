...
class ShowTellModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.img_embed = nn.Linear(self.fc_feat_size, self.input_encoding_size) # 2048, 512
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size) # ((1L, 1L, 512L), (1L, 1L, 512L))
        seq = []
        seqLogprobs = []
        for t in range(self.seq_length + 2):
            if t == 0:
                xt = self.img_embed(fc_feats) # fc_feats: (1L, 2048L), xt: (1L, 512L)
            else:
                if t == 1: # input <bos>
                ...
                xt = self.embed(Variable(it, requires_grad=False)) # (1L, 512L)
            ...
            output, state = self.core(xt.unsqueeze(0), state) # output: (1L, 1L, 512L) state: ((1L, 1L, 512L), (1L, 1L, 512L))
