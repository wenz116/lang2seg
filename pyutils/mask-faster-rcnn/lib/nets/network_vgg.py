...
class Network(nn.Module):
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    # response loss    
    response_pred = self._predictions['response'].squeeze(1).squeeze(0)
    
    response_targets = imresize(np.squeeze(self._gt_masks, 0), response_pred.size(), interp='nearest')
    response_targets = response_targets.astype(np.float32)
    response_targets = Variable(torch.from_numpy(response_targets).cuda())
    
    #print('response_pred.requires_grad:', response_pred.requires_grad) # True
    #print('response_targets.requires_grad:', response_targets.requires_grad) # False
    
    loss_response = F.binary_cross_entropy_with_logits(response_pred, response_targets)
    ...
    self._losses['loss_response'] = loss_response

    loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + loss_response
  ...
  def _predict(self):
    ...
    net_conv = self._image_to_head() # acitvated by ReLU
    self._predictions['net_conv_before'] = net_conv
    ...
    #print('net_conv:', net_conv.size()) # (1L, 512L, 37L, 56L)
    ...
    #print('net_conv: -------', torch.min(net_conv).data.cpu().numpy(), torch.max(net_conv).data.cpu().numpy(), torch.sum(net_conv).data.cpu().numpy())
    ...
    self._predictions['response'] = response ####
    
    net_conv = net_conv * F.sigmoid(response) #### domain of net_conv need to be positive?
    ...
    #print('net_conv:', net_conv.size())
    #print('net_conv: +++++++', torch.min(net_conv).data.cpu().numpy(), torch.max(net_conv).data.cpu().numpy(), torch.sum(net_conv).data.cpu().numpy())
    #print('response_0:', torch.min(response_0).data.cpu().numpy(), torch.max(response_0).data.cpu().numpy(), torch.mean(response_0).data.cpu().numpy())
    #print('response_1:', torch.min(response_1).data.cpu().numpy(), torch.max(response_1).data.cpu().numpy(), torch.mean(response_1).data.cpu().numpy())
    #print('response_2:', torch.min(response_2).data.cpu().numpy(), torch.max(response_2).data.cpu().numpy(), torch.mean(response_2).data.cpu().numpy())
    #print('response_3:', torch.min(response_3).data.cpu().numpy(), torch.max(response_3).data.cpu().numpy(), torch.mean(response_3).data.cpu().numpy())
    #print('response_4:', torch.min(response_4).data.cpu().numpy(), torch.max(response_4).data.cpu().numpy(), torch.mean(response_4).data.cpu().numpy())
    #print('response_5:', torch.min(response_5).data.cpu().numpy(), torch.max(response_5).data.cpu().numpy(), torch.mean(response_5).data.cpu().numpy())
    #print('response_6:', torch.min(response_6).data.cpu().numpy(), torch.max(response_6).data.cpu().numpy(), torch.mean(response_6).data.cpu().numpy())
    #print('response_filter:', response_filter.data.cpu().numpy())
    #print('response:', torch.min(response).data.cpu().numpy(), torch.max(response).data.cpu().numpy(), torch.mean(response).data.cpu().numpy())
    ...
    if 0:
      ...
      response_n = F.sigmoid(response) #(response - torch.min(response)) / (torch.max(response) - torch.min(response))
      ...
      #print('{}/{}_{}.png'.format(im_dir, self._file_name[:-4], self._sent_id))
    ...
    #rois: (256L, 5L) pool5: (256L, 512L, 7L, 7L)
    
    fc7 = self._head_to_tail(pool5)  # (num_rois, 2048, 7, 7)
    #print('fc7:', fc7.size()) # (256L, 4096L)
  ...
  #def forward(self, image, im_info, dynamic_filter, gt_boxes=None, gt_masks=None, mode='TRAIN'):
  def forward(self, image, im_info, gt_boxes=None, gt_masks=None, labels=None, file_name=None, mode='TRAIN', sent_id=0):
    ...
    net_conv, rois, cls_prob, bbox_pred = self._predict()
    if mode_tmp == 'TEST1':
      return
    ...
    #print('-----------------------')
    #print('net_conv: --------', torch.min(self._predictions['net_conv_before']).data.cpu().numpy(), torch.max(self._predictions['net_conv_before']).data.cpu().numpy(), torch.sum(self._predictions['net_conv_before']).data.cpu().numpy(), self._predictions['net_conv_before'].requires_grad)
    #print('net_conv: ++++++++', torch.min(self._predictions['net_conv']).data.cpu().numpy(), torch.max(self._predictions['net_conv']).data.cpu().numpy(), torch.sum(self._predictions['net_conv']).data.cpu().numpy(), self._predictions['net_conv'].requires_grad)
  ...
  # only useful during testing mode
  #def test_image(self, image, im_info, dynamic_filter): ####
  def test_image(self, blobs):
    ...
    """
    self.eval()
    #self.forward(image, im_info, dynamic_filter, None, None, mode='TEST')
    #self.forward(image, im_info, None, None, labels, file_name, mode='TEST') # what we need
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'], blobs['gt_masks'], blobs['labels'], blobs['file_name'], mode='TEST', sent_id=blobs['sent_id'])
  ...
  def get_summary(self, blobs, idx): #, dynamic_filter):
    self.eval()
    #self.forward(blobs['data'], blobs['im_info'], dynamic_filter, blobs['gt_boxes'], blobs['gt_masks'])
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'][idx:idx+1, :], blobs['gt_masks'][idx:idx+1, :, :], blobs['labels'][idx:idx+1, :], blobs['file_name'])
    self.train()
    summary = self._run_summary_op(True)

    return summary

  def train_step(self, blobs, idx, train_op): #, dynamic_filter):
    #self.forward(blobs['data'], blobs['im_info'], dynamic_filter, blobs['gt_boxes'], blobs['gt_masks']) # what we need
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'][idx:idx+1, :], blobs['gt_masks'][idx:idx+1, :, :], blobs['labels'][idx:idx+1, :], blobs['file_name'])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, loss = self._losses['rpn_cross_entropy'].data[0], \
                                                                          self._losses['rpn_loss_box'].data[0], \
                                                                          self._losses['cross_entropy'].data[0], \
                                                                          self._losses['loss_box'].data[0], \
                                                                          self._losses['loss_response'].data[0], \
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, loss

  def train_step_with_summary(self, blobs, idx, train_op): #, dynamic_filter):
    #self.forward(blobs['data'], blobs['im_info'], dynamic_filter, blobs['gt_boxes'], blobs['gt_masks'])
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'][idx:idx+1, :], blobs['gt_masks'][idx:idx+1, :, :], blobs['labels'][idx:idx+1, :], blobs['file_name'])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, loss = self._losses['rpn_cross_entropy'].data[0], \
                                                                          self._losses['rpn_loss_box'].data[0], \
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_response, loss, summary

  def train_step_no_return(self, blobs, train_op): #, dynamic_filter):
    #self.forward(blobs['data'], blobs['im_info'], dynamic_filter, blobs['gt_boxes'], blobs['gt_masks'])
