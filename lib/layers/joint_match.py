...
from layers.visual_encoder import VisualEncoder
...
class JointMatching(nn.Module):
  ...
  def __init__(self, opt):
    ...
    self.dynamic_fc = nn.Linear(num_layers * num_dirs * hidden_size, opt['C4_feat_dim'])
  ...
  def forward(self, pool5, fc7, C4_feat, labels):
    """
    Inputs:
    - pool5       : (n, pool5_dim, 7, 7)   n: number of bounding box in an image?
    - fc7         : (n, fc7_dim, 7, 7)
    - lfeats      : (n, 5)
    - dif_lfeats  : (n, 25)
    - labels      : (n, seq_len)
    ...
    response = F.conv2d(C4_feat, dynamic_filter)
    C4_cat = torch.cat((C4_feat, response), 1)
    print(hidden.shape, dynamic_filter.shape, C4_feat.shape, response.shape, C4_cat.shape, '====')
    # (1L, 1024L) (1L, 1024L, 1L, 1L) (3L, 1024L, 38L, 60L) (3L, 1L, 38L, 60L) (3L, 1025L, 38L, 60L)
