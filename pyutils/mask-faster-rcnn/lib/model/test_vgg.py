...
def eval_split(loader, model, crit, split, opt, max_per_image=100, thresh=0.):
  ...
  while True:
    ...
    for i in range(labels.shape[0]):
      ...
      seg_dir = 'result_{}'.format(opt['dataset_splitBy'])
      if not os.path.exists(seg_dir):
        os.makedirs(seg_dir)
      image_seg_gt.save('{}/{:0>5d}_{:0>2d}_gt_{}.png'.format(seg_dir, loader.iterators[split], i, file_name[:-4]))
      image_seg_pred.save('{}/{:0>5d}_{:0>2d}_pred_{}.png'.format(seg_dir, loader.iterators[split], i, file_name[:-4]))
      
      gt_sents = utils.decode_sequence(loader.ix_to_word, labels[i:i+1, :].data)
      print('{:0>5d} {:0>2d} {:0>.2f} {:0>.2f}'.format(loader.iterators[split], i, iou*100, I*100/U), gt_sents)
      """
      ####
    ...
    ix1 = bounds['it_max']
    if verbose:
      print('evaluating [%s] ... image[%d/%d]\'s sents, det acc=%.2f%%' % \
    ...
    if finish_flag or bounds['wrapped']:
      break
  return acc/loss_evals, num_sent
