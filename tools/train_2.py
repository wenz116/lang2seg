...
def lossFun(loader, optimizer, model, mm_crit, att_crit, opt, iter):
  ...
  # forward
  tic = time.time()
  #scores, _, sub_attn, _, _, _, _, att_scores = model(Feats['pool5'], Feats['fc7'],
  #                                                    Feats['lfeats'], Feats['dif_lfeats'],
  #                                                    Feats['cxt_fc7'], Feats['cxt_lfeats'],
  #                                                    labels)
  scores, att_scores = model(Feats['pool5'], Feats['fc7'],
                             Feats['mask_feat'], labels)
  loss = mm_crit(scores)
...
def main(args):
  ...
  opt['mask_feat_dim'] = loader.mask_feat_dim ####
