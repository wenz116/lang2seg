    ...
    while iter < max_iters + 1:
      ...
      for idx in range(sent_num):
        ...
        if iter == 1 or iter % 500 == 0: #now - last_summary_time > cfg.TRAIN.SUMMARY_INTERVAL: ####
          ...
          rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, total_loss, summary = \
          ...
          rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, total_loss = \
        ...
        if iter % (cfg.TRAIN.DISPLAY) == 0:
          print('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> loss_box: %.6f\n >>> loss_mask: %.6f\n >>> loss_response: %.6f\n >>> lr: %f' % \
                (iter, max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, lr))
