...
class Network(nn.Module):
  ...
  def _add_losses(self, sigma_rpn=3.0):
    ...
    feats_before_all = self._head_to_tail(self._predictions['net_conv_before'])
    feats_after_all = self._head_to_tail(self._predictions['net_conv'])
    
    fc_feats_before_all = feats_before_all.mean(3).mean(2)
    att_feats_before_all = F.adaptive_avg_pool2d(feats_before_all,[14,14]).permute(0, 2, 3, 1).contiguous() ####
    
    fc_feats_after_all = feats_after_all.mean(3).mean(2)
    att_feats_after_all = F.adaptive_avg_pool2d(feats_after_all,[14,14]).permute(0, 2, 3, 1).contiguous() ####
    
    #print('feats_before_all:', feats_before_all.size()) # (1L, 1024L, 38L, 50L)
    #print('fc_feats_before_all:', fc_feats_before_all.size()) # (1L, 1024L)
    #print('att_feats_before_all:', att_feats_before_all.size()) # (1L, 14L, 14L, 1024L)
    
    #print('feats_after_all:', feats_after_all.size()) # (1L, 1024L, 38L, 50L)
    #print('fc_feats_after_all:', fc_feats_after_all.size()) # (1L, 1024L)
    #print('att_feats_after_all:', att_feats_after_all.size()) # (1L, 14L, 14L, 1024L)
    
    tmp = [self._cap_labels, self._cap_masks]
    tmp = [Variable(torch.from_numpy(_), requires_grad=False).cuda() for _ in tmp]
    cap_labels, cap_masks = tmp
    
    fc_feats = torch.cat((fc_feats_before_all, fc_feats_after_all), 1)
    att_feats = torch.cat((att_feats_before_all, att_feats_after_all), 3)
    ...
    loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + loss_mask + loss_response + self._cap_loss_weight*loss_caption
  ...
  def train_step(self, blobs, idx, train_op):
    self.forward(blobs['data'], blobs['im_info'], blobs['gt_boxes'][idx:idx+1, :], blobs['gt_masks'][idx:idx+1, :, :], blobs['labels'][idx:idx+1, :], blobs['file_name'], blobs['cap_labels'][idx:idx+1, :], blobs['cap_masks'][idx:idx+1, :])
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, loss_caption, loss = self._losses['rpn_cross_entropy'].data[0], \
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, loss_caption, loss
  ...
  def train_step_with_summary(self, blobs, idx, train_op):
    ...
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, loss_mask, loss_response, loss_caption, loss, summary
